...
void Laborator3_Vis2D::Init()
{
	...
	latime = 0.80f;
	radius = 200.0;
	Mesh* circle = Object2D::CreateCircle("circle", corner, radius, glm::vec3(1, 0, 0),true);
	Mesh* square1 = Object2D::CreateSquare("square1", corner, length, latime, glm::vec3(1, 0, 0));
...
void Laborator3_Vis2D::OnInputUpdate(float deltaTime, int mods)
{
	...
	if (window->KeyHold(GLFW_KEY_Z)) {
		logicSpace.width += 0.2f;
		logicSpace.height += 0.2f;
	}
